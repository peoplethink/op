...
class CarInterface(CarInterfaceBase):
  ...
  def get_pid_accel_limits(CP, current_speed, cruise_speed):
    ...
    gas_max_bp = [0., 5., 20., 30., 50., 70., 100, 130.]
    gas_max_v = [1.7, 1.65, 1.6, 1.25, .85, .58, .25, .15]
  ...
  def get_params(candidate, fingerprint=gen_empty_fingerprint(), car_fw=[], disable_radar=False):  # pylint: disable=dangerous-default-value
    ...
    if Params().get("LateralControlSelect", encoding='utf8') == "0":
      if candidate in [CAR.GENESIS, CAR.GENESIS_G80, CAR.GENESIS_EQ900]:
          ...
          ret.lateralTuning.pid.kpV = [0.018, 0.052, 0.057]
    ...
    ret.longitudinalTuning.kpV = [1.25, 1.15, 0.85, 0.78, 0.55, 0.35]
    ...
    ret.stopAccel = 0.0
    ret.stoppingDecelRate = 0.015  # brake_travel/s while trying to stop
